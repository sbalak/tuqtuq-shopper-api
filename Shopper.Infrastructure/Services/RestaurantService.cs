...
namespace Shopper.Infrastructure
{
    public class RestaurantService : IRestaurantService
    {
        ...
        */
        ...
        public async Task<List<RestaurantRecentlyVisitedModel>> GetRestaurantsRecentlyVisited(int? page = 1, int? pageSize = 10)
        {
            int offset = (Convert.ToInt32(page) - 1) * Convert.ToInt32(pageSize);
            int fetch = Convert.ToInt32(page) * Convert.ToInt32(pageSize);

            var restaurants = await _context.Database.SqlQuery<RestaurantRecentlyVisitedModel>($"EXEC [dbo].[GetRestaurantsRecentlyVisited] @offset = {offset}, @fetch = {fetch};").ToListAsync();
            return restaurants;
        }
